...
train_data = train_data[~train_data["skin_color"].isin([1, 2, 3, 4])]
validation_data = pd.read_csv(os.path.join("..", "validation_data.csv"))
validation_data = validation_data[~validation_data["skin_color"].isin([1, 2, 3, 4])]
...
grad_norm_clip = 1
checkpoint_path = (
    f"SkinDesc_vit32b5_skin_{optimizer_type}_{lr}_{scheduler_type}_best.pth"
)
output_filename = f"SkinDesc_vit32b5_skin_{optimizer_type}_{lr}_{scheduler_type}.txt"
best_model_path = (
    f"SkinDesc_vit32b5_skin_{optimizer_type}_{lr}_{scheduler_type}_best_full.pt"
)
