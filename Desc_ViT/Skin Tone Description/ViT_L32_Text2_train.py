...
class SkinDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(dir_path, img_filename)
        image = Image.open(img_path).convert("RGB")
        description = text[0 if int(skin) <= 4 else 1]
        if self.transform:
            image = self.transform(image)
        # print(description)
...
class VitTransformerClassifier(nn.Module):
    def __init__(self):
        ...
        self.vit_model = vit_l_32(weights=ViT_L_32_Weights.DEFAULT).to(device)
...
grad_norm_clip = 1
checkpoint_path = f"text_vit32l2_skin_{optimizer_type}_{lr}_{scheduler_type}_best.pth"
output_filename = f"text_vit32l2_skin_{optimizer_type}_{lr}_{scheduler_type}.txt"
